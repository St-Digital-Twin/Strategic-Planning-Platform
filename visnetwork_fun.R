...
get_graph <- function(){
  ...
  # tar_nodes <- merge(tar_nodes,as.data.table(read.xlsx("1.Data/tar_nodes_en.xlsx"))[,c("id","group","title")],by = "id",all.x=TRUE)
  temp <- data.table(ru = c("Визуализация попарных корреляций", "Визуализация результатов", "Загрузка исходных данных",
                            "Прогноз цены метанола","Визуализация цена метанола","Формирование сценарных условиев",
                            "Подготовка и отправка отчёта"),en = read.xlsx("1.Data/tar_nodes_en.xlsx")$title[which(!is.na(read.xlsx("1.Data/tar_nodes_en.xlsx")$title))]) %>% 
    .[!is.na(en)]
  tar_nodes <- merge(tar_nodes,temp,by.x = "title",by.y = "ru",all.x=TRUE) %>% 
    .[,title := en] %>% 
    .[,-c("en")]
