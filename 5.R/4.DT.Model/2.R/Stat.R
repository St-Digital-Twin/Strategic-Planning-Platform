# Исполняемый скрипт, с альтернативными алгоритмами прогнозирования цены Метанола с использованием метода главных компонент
# Для исполнения необходимо нажи Run или последовательно по каждой строке Ctrl+Enter

# Разработчик ООО "Цифровой Двойник"
# Версия 29.07.2022

# Инициация ####
path0 <- "5.R/4.DT.Model/"
path <- paste0(path0,"3.JavaClasses/StatModel/classes/")
...
brent   <-read.table(paste0(path0, "1.Data/brent.txt"), header = FALSE, sep = " ")
coal    <-read.csv(paste0(path0, "1.Data/coal.txt"), header = FALSE)
gas     <-read.csv(paste0(path0, "1.Data/gas.txt"), header = FALSE, sep = " ")
euro    <-read.csv(paste0(path0, "1.Data/euro.txt"), header = FALSE, sep = " ")
GDP     <-read.csv(paste0(path0, "1.Data/GDP.txt"), header = FALSE, sep = " ") # Накопленным итогом
methanol<-read.csv(paste0(path0, "1.Data/methanol.txt"), header = FALSE, sep = " ")
...

{
  ...
  k2 <- ggplot(model1, aes(x = x, y = value, color = m)) +
    ggtitle("Цена на метанол. Факт и Модель 1") +
    labs(subtitle = "Версия 1.", x = "Период", y = "Цена, USD", color = "Версия")+
...

{
  ...
  k21 <- ggplot(model21, aes(x = x, y = value, color = m)) +
    ggtitle("Прогноз цены на метанол. Факт и Модель 1") +
    labs(subtitle = "Версия 1.", x = "Период", y = "Цена, USD", color = "Версия")+
    theme(text = element_text(family = "Panton"))+
    geom_line()
...

{
  ...
  k3 <- ggplot(model3, aes(x = x, y = value, color = m)) +
    ggtitle("Прогноз цены на метанол. Факт и Модель 2") +
    labs(subtitle = "Версия 1.", x = "Период", y = "Цена, USD", color = "Версия")+
    theme(text = element_text(family = "Panton"))+
    geom_line()
...

{
  ...
  k4 <- ggplot(model4, aes(x = x, y = value, color = m)) +
    ggtitle("Прогноз цены на метанол. Факт и Модель 3") +
    labs(subtitle = "Версия 1.", x = "Период", y = "Цена, USD", color = "Версия")+
    theme(text = element_text(family = "Panton"))+
    geom_line()
